...
#define BME280_WEATHER_MONITORING                      \
  {.config = {.spi3w_en = 0, .t_sb = 0, .filter = 0},  \
   .ctrl_meas = {.mode = 1, .osrs_p = 1, .osrs_t = 1}, \
   .ctrl_hum = {.osrs_h = 1}};
...
typedef struct {
  union {
    ...
    uint8_t val;
  } config;  // The "config" register sets the rate, filter and interface
             // options of the device. WRites to the "config register" in normal
  ...
  union {
    ...
    uint8_t val;
  } ctrl_meas;  // The "ctrl_meas" register sets the pressure and temperature
                // data acquisition options of the device. The register needs to
                // be writtn after changing "ctrl_hum" for the changes to become
                // effective.
  union {
    struct {
    ...
    uint8_t val;
  } ctrl_hum;  // The "ctrl_hum" register sets the humidity data acquisition
