...
esp_err_t mqtt_request_handler(mqtt_req_t *r)
{
    ...
    if (wake_node != NULL)
    {
        if (strcasecmp(wake_node->valuestring, MQTT_ON_STATUS) == 0)
            sensors_set_status(reply_root, true); // wakeup the client
        else if (strcasecmp(wake_node->valuestring, MQTT_OFF_STATUS) == 0)
...
esp_err_t mqtt_homeassistant_handler(mqtt_req_t *r)
{
    ...
    const discovery_string_t fan = {
        ...
        .value_template = VALUE_TEMPLATE1(MQTT_STATUS_KEY),
        .payload_on = MQTT_ON_STATUS,
        .payload_off = MQTT_OFF_STATUS
    };
    ...
    const discovery_string_t pm1 = {
        ...
        .value_template = VALUE_TEMPLATE2(JSON_ROOT_PMS, PMS_PM1_KEY)
    };
